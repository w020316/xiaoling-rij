...
export function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 glass-nav">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all duration-200 ease-out ${
                isActive
                  ? "text-primary scale-110"
                  : "text-muted-foreground hover:text-foreground active:scale-95"
              }`}
            >
              <item.icon size={22} strokeWidth={isActive ? 2.5 : 1.8} className="transition-all duration-200" />
